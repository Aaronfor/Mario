...
function run_query($num){
	...
	if ($num==1) {
		...
		$stmt=$pdo->prepare('SELECT * FROM jugadores WHERE tipo=1');
	}elseif ($num==2) {
...
function add( $id, $name,$posicion,$carrera,$email,$num )
{
	global $pdo;
	#se ejecuta la sentencia
		$stmt=$pdo->prepare("INSERT INTO jugadores (id, nombre, posicion,carrera,email,tipo) VALUES ('{$id}', '{$name}','{$posicion}','{$carrera}','{$email}','{$num}')");
		$stmt->execute();
